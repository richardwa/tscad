...
export function translate(translation: Vec3, s: Shape3): Shape3 {
  ...
  sp.gl = addFunc("float", "vec3 p", `return $1(p-${v3(translation)});`, [
    s.gl,
  ]);
  return sp;
}
  ...
  switch (axis) {
    case "x":
      return wrap(
        ...
        "float",
        "vec3 p",
        `return ($1(vec3(p.x,p.y*${f(cs)}-p.z*${f(sn)},p.y*${f(sn)}+p.z*${f(cs)})));`,
        [s.gl],
      );
    case "y":
      return wrap(
        ...
        "float",
        "vec3 p",
        `return ($1(vec3(p.x*${f(cs)}+p.z*${f(sn)},p.y,p.z*${f(cs)}-p.x*${f(sn)})));`,
        [s.gl],
      );
    case "z":
      return wrap(
        ...
        "float",
        "vec3 p",
        `return ($1(vec3(p.x*${f(cs)}-p.y*${f(sn)},p.x*${f(sn)}+p.y*${f(cs)},p.z)));`,
        [s.gl],
      );
  }
}
