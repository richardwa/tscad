import { wrap, v, f, addFunc } from "./glsl-util";
...
export function translate(translation: Vec3, s: Shape3): Shape3 {
  ...
  sp.gl = addFunc('float', 'vec3 p', `return ${s.gl}(p-${v(translation)});`);
  return sp;
}
...
export function rotate(axis: Axis, degrees: number, s: Shape3): Shape3 {
  ...
  switch (axis) {
    case 'x': return wrap(
      ...
      `return (${s.gl}(vec3(p.x,p.y*${f(cs)}-p.z*${f(sn)},p.y*${f(sn)}+p.z*${f(cs)})));`);
    case 'y': return wrap(
      ([x, y, z]) => s([x * cs + z * sn, y, z * cs - x * sn]), 'float', 'vec3 p',
      `return (${s.gl}(vec3(p.x*${f(cs)}+p.z*${f(sn)},p.y,p.z*${f(cs)}-p.x*${f(sn)})));`);
    case 'z': return wrap(
      ([x, y, z]) => s([x * cs - y * sn, x * sn + y * cs, z]), 'float', 'vec3 p',
      `return (${s.gl}(vec3(p.x*${f(cs)}-p.y*${f(sn)},p.x*${f(sn)}+p.y*${f(cs)},p.z)));`);
  }
}
...
export function scale(d: number, s: Shape3): Shape3 {
  ...
  sp.gl = addFunc('float', 'vec3 p', `return ${s.gl}(p/${f(d)})*${f(d)};`);
  return sp;
}