import { boundsToCorners2, getCenter, getResultIndex, interpolate, splitSquare } from '../util/math';
...
export function slice(p: Props): Square3[] {
  ...
  const bottom = p.height instanceof Array ? p.height[0] : 0;
  ...
  for (let h = bottom + halfHeight; h < top; h += layerHeight) {
    ...
    const lines = squares.map(s => marchSquare(s, shape2));
    lines.forEach(line => {
      if (line) {
        const [p1, p2] = line;
        quads.push([
