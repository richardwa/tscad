 ...
 */
export const edges: Array12<Vec2> = [
  [0, 1],
  [1, 3],
  [3, 2],
  [2, 0],
...
const swapBitsFN = (n: number, m: number) => {
  const maskN = 1 << n
  const notMaskN = ~maskN
  const maskM = 1 << m
  const notMaskM = ~maskM
  return (i: number) => {
    const bitN = (i & maskN) > 0
    const bitM = (i & maskM) > 0
    if (bitN !== bitM) {
      if (bitN) {
...
const swapBits23 = swapBitsFN(2, 3)
const swapBits67 = swapBitsFN(6, 7)
const swapBits = (i: number) => swapBits67(swapBits23(i))
 ...
 */
const bourkeEdgeTable = new Uint32Array([
 ...
 */
export const edgeTable = bourkeEdgeTable.map((v, i) => bourkeEdgeTable[swapBits(i)])

/**
 * index = 8 bits which indicate positive or negative implicit function value at each corner
 * return triplet[] which indicates sides of the cube involved in triangle
 */
export const bourkeTriTable = [
  [],
  [0, 8, 3],
  [0, 1, 9],
  [1, 8, 3, 9, 8, 1],
 ...
 * for convertion flip position 3 and 2,  flip 6 and 7
 */
export const triTable = bourkeTriTable.map((v, i) => bourkeTriTable[swapBits(i)])
