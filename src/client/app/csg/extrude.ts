...
export function revolve(axis: Axis, offset: number, s: Shape2): Shape3 {
  switch (axis) {
    case 'x':
      return wrap(
        (p) => s([V2.length([p[1], p[2]]) - offset, p[0]]),
        'float',
        'vec3 p',
        `return $1(vec2(length(p.yz)-${f(offset)},p.x));`,
        [s.gl]
      )
    case 'y':
      return wrap(
        (p) => s([V2.length([p[0], p[2]]) - offset, p[1]]),
        'float',
        'vec3 p',
        `return $1(vec2(length(p.xz)-${f(offset)},p.y));`,
        [s.gl]
      )
    case 'z':
      return wrap(
        (p) => s([V2.length([p[0], p[1]]) - offset, p[2]]),
        'float',
        'vec3 p',
        `return $1(vec2(length(p.xy)-${f(offset)},p.z));`,
        [s.gl]
      )
  }
}
...
export function tile(o: TileParams, s: Shape3): Shape3 {
  ...
  return wrap(
    (p) => s([tileX(p[0]), tileY(p[1]), tileZ(p[2])]),
    'float',
    'vec3 p',
    [
